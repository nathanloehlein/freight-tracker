...
export const InfoCard = (props) => {
  ...
  const aggDataObjKeys = Object.keys(aggDataObj).sort((a, b) =>
    a > b ? 1 : -1
  );

  const applyFilter = (columnName, value) => {
    const index = state.columnData.findIndex(
      (column) => column.label === columnName
    );
    dispatch({
      type: state.filters[index].includes(value)
        ? "FILTER_REMOVED"
        : "FILTER_ADDED",
      payload: {
        index: index,
      ...
      <Grid container direction={"column"} spacing={2}>
        {aggDataObjKeys.length > 0 &&
          aggDataObjKeys.map((key, index) => {
            ...
            const statusInfo = (
              <StatusIcon status={useAbbv ? key.charAt(0) : key} />
            );
            return (
              <Grid
                item
