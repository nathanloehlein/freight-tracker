...
export const extractColumns = (state) =>
  ...
  state.shipmentData.data[0].map((value, index) => {
    ...
    return {
      ...
      label: value,
      options: {
        filterList: filterValues && filterValues.length > 0 ? filterValues : [],
      },
    };
  });
...
export const extractRows = (data, columnData) =>
  data
    .slice(1)
    .map((row) =>
      row.reduce((acc, value, index) => ({ ...acc, [columnData[index].name]: value }), {})
    );

export const getRowData = (state, dispatch) => {
  const rowData = state.columnData && extractRows(state.shipmentData.data, state.columnData);
    ...
    if (columns.includes(column.name)) {
      column.options = {
        ...
        customBodyRenderLite: (dataIndex) => {
          ...
          const urlSafeAddress = encodeURIComponent(address).replace(/\s/g, '+');
          return (
            <Link
...
export const addCustomOptionsToColumns = (columnData, rowData) => {
  let optionsAdded = addCustomIconsToColumns(columnData, rowData, ['status', 'mode']);
  optionsAdded = addGoogleMapsLinksToColumns(optionsAdded, rowData, ['origin', 'destination']);
  return optionsAdded;
};
...
export const getColumnAggData = (state, columnName) => {
  ...
  if (state.rowData && state.columnData) {
    state.rowData.forEach((row) =>
      counts[row[columnName]] ? counts[row[columnName]]++ : (counts[row[columnName]] = 1)
    );
  }
  return counts;
};
